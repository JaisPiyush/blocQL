import { BroadcastData, SolanaBlockMessage, SolanaDatBroadcastType, SolanaDatastoreName } from "../types";
...
export class SolanaBlockProcessor extends SolanaProcessor<SolanaBlockMessage> {
    protected async __process(data: BroadcastData<SolanaBlockMessage>)
        : Promise<void> {
            
            const datastore = 
                await this.providers.datastoreProvider(SolanaDatastoreName.BlockDatastore);
            const logger = this.providers.logProvider();
            const serviceProvider = await this.providers.serviceProvider();
            const dataBroadcasterProvider = await this.providers.dataBroadcasterProvider();
            ...
            for (const txn of block.transactions) {
                ...
                logger.info(`Broadcasted transaction ${signature} for block ${data.payload.slot}`)
            }
    ...
    private async __processReward(data: BroadcastData<SolanaBlockMessage>) {
        const datastore = 
            await this.providers.datastoreProvider(SolanaDatastoreName.RewardDatastore);
        const logger = this.providers.logProvider();
        ...
        const rewards: SolanaRewardModel[] = data.payload.rewards.map(reward => {
            const preBalance = (reward.postBalance === null) ? 0: reward.postBalance - reward.lamports;
            return {
        ...
        logger.info(`Inserted ${rewards.length} rewards into datastore for block ${data.payload.slot}`);
    }
}