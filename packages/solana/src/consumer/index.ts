...
import { SolanaBlockProcessor, SolanaTransactionProcessor } from '../processors';
...
export const blockSQSConsumer = (providers: SolanaProcessorProvider, queueUrl?: string) => {
    ...
    if (providers.logProvider) {
        providers.logProvider().info('Started solana block consumer, waiting for messages')
    }
    return consumer;
}
...
export const txnSQSConsumer = (providers: SolanaProcessorProvider,queueUrl?: string) => {
    ...
    if (providers.logProvider) {
        providers.logProvider().info('Started solana txn consumer, waiting for messages')
    }
