...
import {
    BlockchainScanner,
    BlockchainScannerProviders,
    Options,
} from '../scanner/blockchain_scanner';
import {
    MockClient,
    MockEvent,
    MockEventPayloads,
        ...
        try {
            if (this.latestBlockHeight) {
                ...
                let endHeight = Math.min(
                    ...
                    startHeight + this.maxFetchSize - 1
                ); // try to fetch max blocks
                endHeight = Math.min(
                    endHeight,
                    this.processedBlockHeight + this.maxBlocksAhead
                ); // respect max lookahead

                if (startHeight <= endHeight) {
                    const flowService =
                        (await this.providers.serviceProvider()) as MockClient;
                    ...
                    while (true) {
                        ...
                        try {
                            ...
                            } = _.groupBy(events, (e) => String(e.blockHeight));
                            ...
                            for (let i = startHeight; i <= endHeight; ++i) {
                                ...
                                eventBus.emit<MockEventPayloads.MockEventsFetched>(
                                    MockEventType.MockEventsFetched,
                                    {
                            ...
                            logger.error(
                                `Error fetching events ${this.eventType} (${startHeight} - ${endHeight}): ${err}`
                            );
                            ++errors;
                            if (fetchSize > 1) {
                                // sometimes there are too many events in the response for the access nodes to handle, try to back off on our fetch size
                                fetchSize = Math.max(
                                    Math.floor(fetchSize / 4),
                                    1
                                ); // try to fetch less blocks at a time
