...
export class TicketThrottler implements TicketThrottlerInterface {
    ...
    constructor(
        ...
        private onTicketsReserved?: (count: number) => void
    ) {
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            const fn = () => {
                if (this.reserveTickets(count)) {
                    ...
                    maxWaitMs !== undefined &&
                    new Date().getTime() - startTime > maxWaitMs
                ) {
    ...
    reserveTickets = (count: number, force = false): boolean => {
        ...
        if (this.lastDecayTime < cur) {
            this.issuedTickets = Math.max(
                this.issuedTickets -
                    ((cur - this.lastDecayTime) / 1000) * this.decayPerSecond,
                0
            );
        ...
        if (
            ...
            (this.issuedTickets > 0 &&
                this.issuedTickets + count > this.maxTickets)
        ) {
            return false;
        }
